...
def partition_data(dataset, num_clients=10, partition="equal", alpha=0.5):
    ...
    client_cls_counts = record_net_data_stats(dataset.targets, client_idx_dict)
    return client_idx_dict, client_cls_counts


def get_mnist_iid(dataset, mode="homo", num_clients=10):
    """
    @param mode: iid mode - equal or homo
    """
    assert mode in ["equal", "homo"]
    return partition_data(dataset, num_clients=num_clients, partition=mode)


def get_mnist_noniid(dataset, mode="dir", num_clients=10):
    """
    @param mode: iid mode - dir or cls
    """
    assert mode in ["dir", "cls"]
    return partition_data(dataset, num_clients=num_clients, partition=mode, alpha=0.5)


def get_cifar10_iid(dataset, mode="homo", num_clients=100):
    """
    @param mode: iid mode - equal or homo
    """
    assert mode in ["equal", "homo"]
...
def get_cifar10_noniid(dataset, mode="dir", num_clients=100, alpha=0.5):
    """
    @param mode: iid mode - dir or cls
    """
    assert mode in ["dir", "cls"]
...
if __name__ == '__main__':
    ...
    client_idx_dict, client_cls_counts = get_mnist_noniid(mnist_train_ds, mode="dir", num_clients=10)
...
#     import pprint
#     X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset='mnist', partition='hetero-dir', n_nets=10, alpha=0.5)
