...
from sampling import get_mnist_iid, get_mnist_noniid, get_cifar10_iid, get_cifar10_noniid
...
if __name__ == "__main__":
    ...
    parser.add_argument('--iid', type=bool, default=False, help='iid or non-iid, default: True')
    parser.add_argument('--distribution', type=str, default='dirichlet', help='label distribution', choices=["uniform", "equal", "dirichlet", "partial"])
    ...
    if args.dataset == 'mnist':
        ...
        dataset_train = torchvision.datasets.MNIST('../Data/mnist/', train=True, download=True, transform=transform_train_mnist)
        dataset_test = torchvision.datasets.MNIST('../Data/mnist/', train=False, download=True, transform=transform_test_mnist)
        if args.iid:
            assert args.distribution in ["uniform", "equal"]
            client_idx_dict, client_cls_counts = get_mnist_iid(dataset_train, distribution=args.distribution, num_clients=args.num_clients)
        else:
            assert args.distribution in ["dirichlet", "partial"]
            client_idx_dict, client_cls_counts = get_mnist_noniid(dataset_train, distribution=args.distribution, num_clients=args.num_clients)
        ...
        if args.iid:
            assert args.distribution in ["uniform", "equal"]
            client_idx_dict, client_cls_counts = get_cifar10_iid(dataset_train, distribution=args.distribution, num_clients=args.num_clients)
        else:
            assert args.distribution in ["dirichlet", "partial"]
            client_idx_dict, client_cls_counts = get_cifar10_noniid(dataset_train, distribution=args.distribution, num_clients=args.num_clients)
        # dataloader_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.train_batch_size, shuffle=True)
        # dataloader_test = torch.utils.data.DataLoader(dataset_test, batch_size=args.test_batch_size, shuffle=False)
    else:
        exit('Error: unrecognized dataset')
    import pprint
    pprint.pprint(client_cls_counts)